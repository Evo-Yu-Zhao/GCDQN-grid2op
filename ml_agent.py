...
class ReplayBuffer:
    ...
    def add(self, s, a, r, d, s2):
        """向经验回放池添加sards
        S——current state, a——action, r——reward, d——done, s2——next state"""
        ...
        if self.count < self.buffer_size:
            ...
            self.buffer.popleft() #获取最左边的元素，并在队列中删除（即新的experience挤掉了回放池中最老的experience）
            self.buffer.append(experience)

    def size(self): #返回经验回放池当前所包含的经验条数
        return self.count

    def sample(self, batch_size):
        """从经验回放池中取出batch_size条或全部经验"""
        batch = []
...
class RLQvalue(object):
    ...
    - target_model: which has the same architecture and same initial weights as "model"
                    but is updated less frequently to stabilize training
    """
    def __init__(self, action_size, observation_size,
        ...
        self.lr_ = lr
        self.qvalue_evolution = np.zeros((0,))  #空的array
        self.training_param = training_param  #TrainingParam()类
        self.model = None
    ...
    def predict_movement(self, data, epsilon):
        ...
        rand_val = np.random.random(data.shape[0]) #取随机数
...
# This class implements the "Soft Actor Critic" model.
